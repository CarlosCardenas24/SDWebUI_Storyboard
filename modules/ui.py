...
def create_ui():
    ...
    # Storyboarding UI ----- New section
    with gr.Blocks(analytics_enabled=False) as storyboard_interface:
        toprow = ui_toprow.Toprow(is_img2img=False, is_compact=shared.opts.compact_prompt_box)

        dummy_component = gr.Label(visible=False)

        extra_tabs = gr.Tabs(elem_id="storyboard_extra_tabs", elem_classes=["extra-networks"])
        extra_tabs.__enter__()

        with gr.Tab("Generation", id="storyboard_generation") as storyboard_generation_tab, ResizeHandleRow(equal_height=False):
            with ExitStack() as stack:
                if shared.opts.txt2img_settings_accordion:
                    stack.enter_context(gr.Accordion("Open for Settings", open=False))
                stack.enter_context(gr.Column(variant='compact', elem_id="storyboard_settings"))
                ...
                for category in ordered_ui_categories():
                    if category == "prompt":
                        prompt_inputs = [
                            gr.Textbox(label=f"Prompt {i+1}", lines=3, placeholder=f"Enter description for Frame {i+1}")
                            for i in range(6)
                        ]
                        #toprow.create_inline_toprow_prompts()
                        ...
                        with FormRow():
                            with gr.Column(elem_id="storyboard_column_size", scale=4):
                                width = gr.Slider(minimum=64, maximum=2048, step=8, label="Width", value=512, elem_id="storyboard_width")
                                height = gr.Slider(minimum=64, maximum=2048, step=8, label="Height", value=512, elem_id="storyboard_height")

                            with gr.Column(elem_id="storyboard_dimensions_row", scale=1, elem_classes="dimensions-tools"):
                                res_switch_btn = ToolButton(value=switch_values_symbol, elem_id="storyboard_res_switch_btn", tooltip="Switch width/height")

                            if opts.dimensions_and_batch_together:
                                with gr.Column(elem_id="storyboard_column_batch"):
                                    batch_count = gr.Slider(minimum=1, step=1, label='Batch count', value=1, elem_id="storyboard_batch_count")
                                    batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='Batch size', value=1, elem_id="storyboard_batch_size")

                    elif category == "cfg":
                        with gr.Row():
                            cfg_scale = gr.Slider(minimum=1.0, maximum=30.0, step=0.5, label='CFG Scale', value=7.0, elem_id="storyboardg_cfg_scale")
                        ...
                        with gr.Row(elem_id="storyboard_accordions", elem_classes="accordions"):
                            with InputAccordion(False, label="Hires. fix", elem_id="storyboard_hr") as enable_hr:
                                with enable_hr.extra():
                                    hr_final_resolution = FormHTML(value="", elem_id="txtimg_hr_finalres", label="Upscaled resolution", interactive=False, min_width=0)

                                with FormRow(elem_id="storyboard_hires_fix_row1", variant="compact"):
                                    hr_upscaler = gr.Dropdown(label="Upscaler", elem_id="storyboard_hr_upscaler", choices=[*shared.latent_upscale_modes, *[x.name for x in shared.sd_upscalers]], value=shared.latent_upscale_default_mode)
                                    hr_second_pass_steps = gr.Slider(minimum=0, maximum=150, step=1, label='Hires steps', value=0, elem_id="storyboard_hires_steps")
                                    denoising_strength = gr.Slider(minimum=0.0, maximum=1.0, step=0.01, label='Denoising strength', value=0.7, elem_id="storyboard_denoising_strength")

                                with FormRow(elem_id="storyboard_hires_fix_row2", variant="compact"):
                                    hr_scale = gr.Slider(minimum=1.0, maximum=4.0, step=0.05, label="Upscale by", value=2.0, elem_id="storyboard_hr_scale")
                                    hr_resize_x = gr.Slider(minimum=0, maximum=2048, step=8, label="Resize width to", value=0, elem_id="storyboard_hr_resize_x")
                                    hr_resize_y = gr.Slider(minimum=0, maximum=2048, step=8, label="Resize height to", value=0, elem_id="storyboard_hr_resize_y")

                                with FormRow(elem_id="storyboard_hires_fix_row3", variant="compact", visible=opts.hires_fix_show_sampler) as hr_sampler_container:
                                ...
                                with FormRow(elem_id="storyboard_hires_fix_row4", variant="compact", visible=opts.hires_fix_show_prompts) as hr_prompts_container:
                                    with gr.Column(scale=80):
                                        with gr.Row():
                                            hr_prompt = gr.Textbox(label="Hires prompt", elem_id="hires_prompt", show_label=False, lines=3, placeholder="Prompt for hires fix pass.\nLeave empty to use the same prompt as in first pass.", elem_classes=["prompt"])
                                    with gr.Column(scale=80):
                        ...
                        if not opts.dimensions_and_batch_together:
                            with FormRow(elem_id="storyboard_column_batch"):
                                batch_count = gr.Slider(minimum=1, step=1, label='Batch count', value=5, elem_id="storyboard_batch_count")
                                batch_size = gr.Slider(minimum=1, maximum=8, step=1, label='Batch size', value=1, elem_id="storyboard_batch_size")

                    elif category == "override_settings":
                        with FormRow(elem_id="storyboard_override_settings_row") as row:
                            override_settings = create_override_settings_dropdown('storyboard', row) or {}

                    elif category == "scripts":
                        with FormGroup(elem_id="storyboard_script_container"):
                            custom_inputs = scripts.scripts_txt2img.setup_ui()
            ...
            txt2img_args = dict(
                fn=wrap_gradio_gpu_call(lambda *args: [modules.txt2img.txt2img(p, *args[1:]) for p in args[:6] if p], extra_outputs=[None, '', '']),
                _js="submit",
                inputs=txt2img_inputs + [override_settings or {}],
                outputs=txt2img_outputs,
                show_progress=False,
            )
            ...
            res_switch_btn.click(fn=None, _js="function(){switchWidthHeight('storyboard')}", inputs=None, outputs=None, show_progress=False)

            toprow.restore_progress_button.click(
                fn=progress.restore_progress,
                _js="restoreProgressstoryboard",
                inputs=[dummy_component],
                outputs=[
                    output_panel.gallery,
                    output_panel.generation_info,
            ...
            parameters_copypaste.add_paste_fields("storyboard", None, txt2img_paste_fields, override_settings)
            parameters_copypaste.register_paste_params_button(parameters_copypaste.ParamBinding(
                paste_button=toprow.paste, tabname="storyboard", source_text_component=toprow.prompt, source_image_component=None,
            ))
        ...
        extra_networks_ui = ui_extra_networks.create_ui(storyboard_interface, [storyboard_generation_tab], 'storyboard')
        ui_extra_networks.setup_ui(extra_networks_ui, output_panel.gallery)
    ...
    interfaces = [
        ...
        (storyboard_interface, "Storyboarding", "storyboarding"),
    ]
